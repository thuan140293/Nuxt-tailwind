<template>
  ...
  <nav class="navbar-wrapper">
    <div class="navbar-spacing sm:px-6 lg:px-8">
      <div class="relative flex items-center justify-between h-16">
        <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
          <!-- Mobile menu button-->
          <button type="button" @click="isMobile = !isMobile" class="btn btn-collapsed hover:text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
         ...
         <transition
              enter-active-class="transition ease-out duration-50 transform"
          ...
          <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div class="hidden sm:block sm:ml-6">
              <div class="flex space-x-4">
                <a href="javascript:;" v-for="(item, key) in dummyData.NAVBAR_ITEMS" :key="key" class="text-cyan-300 hover:bg-cyan-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{ item }}</a>
              </div>
            </div>
          </div>
         </transition>
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          <button class="bg-cyan-700 p-1 rounded-full text-white hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
